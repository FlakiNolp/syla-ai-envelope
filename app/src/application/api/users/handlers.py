...
async def sign_up(credentials: CredentialsUserRequestSchema, background_tasks: BackgroundTasks,
    ...
    try:
        ...
        if not user_exists:
            ...
            background_tasks.add_task(mediator.handle_command, RegistrateUserCommand(credentials.username,
                                                                                     credentials.password))
            return Response(status_code=status.HTTP_202_ACCEPTED)
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail={"error": "Пользователь с такой почтой уже существует"})
