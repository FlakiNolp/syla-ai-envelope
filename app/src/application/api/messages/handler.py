...
async def post_message(data: ReceivedMessageRequestSchema, access_token: AccessToken = Depends(auth_user), container=Depends(_init_container)):
    try:
...
async def get_message(data: GetHistoryRequestSchema = Depends(), access_token: AccessToken = Depends(auth_user), container=Depends(_init_container)):
    try:
        mediator: Mediator = container.resolve(Mediator)
        messages: list[Message] = (await mediator.handle_query(GetHistoryMessages(user_id=access_token.sub_id, chat_id=data.chat_id)))
    except ApplicationException as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail={"error": e.message})
