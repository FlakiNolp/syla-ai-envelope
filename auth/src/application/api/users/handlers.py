...
from application.api.users.schemas import (
    ...
    RefreshAuthenticateUserResponseSchema,
...
oauth2_scheme = OAuth2PasswordBearer(
    tokenUrl="/token", scopes={"read_api": "read-api", "write-api": "write-api", "post-api": "post new items"}
)
...
@router.post(
    ...
    description="Эндпоинт верифицирует пользователя и возвращает пару токенов access-token и refresh-token",
    responses={
...
async def token(
    form_data: Annotated[OAuth2PasswordRequestForm, Depends()], container: Container = Depends(_init_container)
):
    try:
        mediator: Mediator = container.resolve(Mediator)
        pair_tokens: PairTokens = (
            await mediator.handle_command(
                AuthenticateUserCommand(email=form_data.username, password=form_data.password)
            )
        )[0]
...
@router.post(
    "/refresh",
    status_code=status.HTTP_200_OK,
    description="Эндпоинт верифицирует refresh-token и возвращает access-token",
    responses={
...
async def refresh(
    refresh_token: RefreshAuthorizationUserRequestSchema, container: Container = Depends(_init_container)
):
    try:
        mediator: Mediator = container.resolve(Mediator)
